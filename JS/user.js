document.addEventListener("DOMContentLoaded", () => {
      ...
      output += `
            <div class="container">
                <div class="products">
                    ...
                    <h3>${name}</h3>
                    <p>${price}</p>
                    <button type="button" class="add-to-cart" product-id="${id}" product-name="${name}" product-price="${price}" data-cy="addToCartBtn">Add to Cart</button>
                </div>
            </div>
        `;
    });
  ...
  function updateCart() {
    ...
    cart.forEach(({ id, name, price, quantity }) => {
      output += `
            <div class="cart-item" data-cy="cart">
                <h4>${name}</h4>
                <p>${price}</p>
  ...
  function checkout() {
    ...
    checkoutButton.addEventListener(
      ...
      () => {
        if (cart.length === 0){
            displayMessage("Your cart is empty. Please add items to the cart before checking out.", true);
            return
        }
        displayMessage("Thank you for shopping with us. Your order is being processed.");
        cart = [];
        updateCart();
      },
      { once: true }
